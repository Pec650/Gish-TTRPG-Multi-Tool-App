...
public partial class CRCalculatorView : ContentView
{
    ...
    private void OnBackClicked(object sender, EventArgs e)
    {
        ...
        while (current is not null && current is not ToolsView)
            current = current.Parent;
        if (current is ToolsView host)
            host.PopLocalView();
    }
